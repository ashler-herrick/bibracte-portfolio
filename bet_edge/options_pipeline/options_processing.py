...
def process_datetime_columns(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.with_columns(
        [
            ((pl.col("expiration_dt") - pl.col("window_start_dt")).cast(pl.Int64) / ns_in_day).alias("DTE"),
            (((pl.col("expiration_dt") - pl.col("window_start_dt")).cast(pl.Int64) / ns_in_day) / 365.0).alias("t"),
        ]
    )
